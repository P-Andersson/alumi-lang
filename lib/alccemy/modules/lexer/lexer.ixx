...
export namespace alccemy {
   ...
   template <typename TokenSet, LexerPattern PatternT>
   class Tokenize
   {
   public:
      Tokenize(const PatternT& pattern, Token<TokenSet>::Type type)
         : m_pattern(pattern)
         , m_token_type(type)
      {
      ...
      Token<TokenSet> make_token(size_t line, size_t start, size_t end, size_t start_string_index) const
      {
         return Token<TokenSet>(m_token_type, TextPos(line, start, start_string_index), end - start);
      }

   private:
      PatternT m_pattern;
      Token<TokenSet>::Type m_token_type;
   };
   ...
   public:
      ...
      LexerFailure(Reason reason, const Tokens<TokenSet>& tokens_so_far, const TextPos& text_pos, size_t data_index);

      const char* what() const noexcept override;

      Tokens<TokenSet> tokens_so_far;
      TextPos text_pos;
      size_t data_index;
      Reason reason;
   ...
   template <typename TokenSet, LexerPattern... PatternTs>
   class Lexer
   {
   public:
      ...
      TokenizedText<TokenSet> lex(const std::vector<UnicodeCodePoint>& text)
      {
         ...
         while (pos < text.size())
         {
            ...
            if (character == 0x0a) // Newline
            {
               ...
               tokens.push_back(Token<TokenSet>(Token<TokenSet>::Type::Linebreak, TextPos(line, pos - line_start, pos), 1));
               line_start = pos + 1;
               line += 1;
               pos += 1;
               is_indenting = true;
            ...
            if (is_indenting)
            {
               ...
               if (indent_char != character)
               {
                  ...
                  {
                     throw LexerFailure<TokenSet>(LexerFailure<TokenSet>::Reason::MismatchedIndentionCharacters, tokens, TextPos(line, pos - line_start, cur_token_start), pos);
                  }
                  tokens.push_back(Token<TokenSet>(Token<TokenSet>::Type::Indent, TextPos(line, 0, line_start), pos - line_start));
                  is_indenting = false;
                  cur_token_start = pos;
               }
               else
         ...
         if (tokens.size() == 0 || tokens.back().type() != Token<TokenSet>::Type::Linebreak)
         {
            tokens.push_back(Token<TokenSet>(Token<TokenSet>::Type::Linebreak, TextPos(line, cur_token_start - line_start, cur_token_start), 0));
         }
         tokens.push_back(Token<TokenSet>(Token<TokenSet>::Type::EndOfFile, TextPos(line, pos - line_start, pos), 0));
         return TokenizedText<TokenSet>(text, tokens);
      }
   private:
      class CompletePattern
      {
      public:
         CompletePattern(size_t end_pos, const std::optional<Token<TokenSet>>& token)
            : end_pos(end_pos)
            , token(token)
         {
         ...
         size_t end_pos;
         std::optional<Token<TokenSet>> token;
      };
      ...
      std::optional<Token<TokenSet>> make_token(const T& pattern, size_t line, size_t col_start, size_t col_end, size_t string_index)
      {
      ...
      inline typename std::enable_if<I == sizeof...(PatternTs), LexerResult>::type
         handle_codepoint(UnicodeCodePoint cp, size_t token_start_index, size_t index, Tokens<TokenSet>& tokens, size_t line, size_t col_start, size_t col_end)
      {
      ...
      template<std::size_t I = 0, typename... Tp>
      inline typename std::enable_if<I == sizeof...(PatternTs), LexerResult>::type
         terminate_codepoint(size_t token_start_index, size_t index, Tokens<TokenSet>& tokens, size_t line, size_t col_start, size_t col_end)
      {
         return LexerResult(LexerResults::Failed, 0);
      }
