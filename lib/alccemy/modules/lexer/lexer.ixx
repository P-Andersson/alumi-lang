 ...
 private:
   ...
   std::expected<TokenizedText<TokenSetT>, ErrorType> lex(const std::string& src_text) const {
             ...
             tuple_for(m_rules, [&, this]<size_t... rule_indicies>(std::index_sequence<rule_indicies...>) {
                ...
                (
                    [&, this]<size_t index = rule_indicies>() {
                       ...
                       ExpectedRulesResultT res = std::get<index>(m_rules).handle_code_point(
            ...
            tuple_for_each(patterns, [&](auto& pattern, size_t patter_index) {
               if (!state.done[patter_index]) {
                  state.apply_lexer_result<step_back_f>(pattern.check(codepoint, state.consumed_cps_for_current_token),
                                                        next_pos, state.current_token_start, src_text, pattern,
                                                        patter_index);
               }
            });
         }
      ...
      tuple_for(m_rules, [&, this]<size_t... rule_indicies>(std::index_sequence<rule_indicies...>) {
         (
             [&, this]<size_t index = rule_indicies>() {
                std::get<index>(m_rules).end_lexing(std::get<index>(rules_states), state.tokens, state.text_position);
             }(),
             ...);
      });
   ...
   class CompletePattern {
    public:
      CompletePattern(TextPos end_pos, const std::optional<Token<TokenSetT>>& token) : end_pos(end_pos), token(token) {}
   ...
   class TokenizationState {
    public:
      ...
      template <void (*step_back_f)(const std::string&, size_t&), LexerPattern PatternT>
      void apply_lexer_result(const LexerResult& res, const TextPos& next_position, const TextPos& current_token_start,
                              const std::string& source_text, const PatternT& pattern, size_t pattern_index) {
         if (res.type == LexerResults::Completed) {
            ...
            auto backtracked =
                backtrack<step_back_f>(next_position, res.backtrack_cols, line_length_stack, source_text);
