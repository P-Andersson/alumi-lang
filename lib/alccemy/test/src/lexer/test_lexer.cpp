...
TEST_CASE("Basics") {
   SECTION("Simple Lexicon") {
      auto lexer = create_lexer<TestLexicon>(PatternSet{Text("A"), Text("B"), Text("C")});

      SECTION("AllOk") { REQUIRE_NOTHROW(lexer.lexUtf8("ABC")); }
   }
   SECTION("Tokenizes") {
      auto lexer = create_lexer<TestLexicon>(PatternSet{
          Tokenize(Text("A"), TestLexicon::A), Tokenize(Text("B"), TestLexicon::B), Tokenize(Text("C"), TestLexicon::C),
          Tokenize(Text(as_utf8(0x0001F0A1)), TestLexicon::Ace), Tokenize(Text("\n"), TestLexicon::Linebreak)});
   ...
   SECTION("Mixed") {
      auto lexer = create_lexer<TestLexicon>(PatternSet{Tokenize(Repeats(Text("A")), TestLexicon::A),
                                                        Tokenize(Text("B"), TestLexicon::B),
                                                        Tokenize(Text("C"), TestLexicon::C), Text(" ")
   ...
   SECTION("Indent") {
      auto lexer =
          create_lexer<TestLexicon>(RuleSet{IndentionRule<TestLexicon, TestLexicon::Indent, TestLexicon::Dedent>()},
                                    PatternSet{
   ...
   SECTION("Longest/Shortest") {
      auto lexer = create_lexer<TestLexicon>(PatternSet{Tokenize(Repeats<Text, 1>(Text("A")), TestLexicon::A),
                                                        Tokenize(Text("AB"), TestLexicon::B),
                                                        Tokenize(Text("B"), TestLexicon::C), Text(" ")});
   ...
   SECTION("Sad Path") {
      auto lexer =
          create_lexer<TestLexicon>(RuleSet{IndentionRule<TestLexicon, TestLexicon::Indent, TestLexicon::Dedent>()},
                                    PatternSet{Tokenize(Text("A"), TestLexicon::A), Tokenize(Text("B"), TestLexicon::B),
                                               Tokenize(Text("\n"), TestLexicon::Linebreak), Text(" ")});
