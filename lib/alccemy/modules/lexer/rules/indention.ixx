...
class MixedIndentionCharactersError {
 public:
   std::string description() const noexcept {
      return "Different indention character used for indention in the same "
             "source, these cannot be mixed.";
   }
};

class MismatchedIndentionError {
 public:
   std::string description() const noexcept { return "Dedention to an unknown indention layer"; }
};

template <TokenSet TokenSetT, TokenSetT indention_token, TokenSetT dedention_token> class IndentionRule {
 public:
   ...
   IndentionRuleState initial_state() const {
      ...
      state.indention_stack.push_back(0); // This should never be removed as we only pop from the stack if
                                          // indention is less than the stacks value, which is impossible
      return state;
   }

   std::expected<RulesResult, ErrorType> handle_code_point(IndentionRuleState& state, Tokens<TokenSetT>& tokens,
                                                           const UnicodeCodePoint& cp, const TextPos& position) const {
      // Newline means new indention
      if (cp == '\n') {
         state.current_indention = 0;
         state.indention_start = std::nullopt;
         return RulesResult::TerminateTokens; // We do not allow any token to
                                              // break past indentions,
      ...
      if (state.current_indention &&
          std::find(m_indention_chars.begin(), m_indention_chars.end(), cp) != m_indention_chars.end()) {
      ...
      if (state.current_indention) {
         if (state.indention_stack.back() != state.current_indention) {
            if (state.current_indention > state.indention_stack.back()) {
               tokens.push_back(
                   Token<TokenSetT>(indention_token, state.indention_start.value(), state.current_indention.value()));
               state.indention_stack.push_back(*state.current_indention);
            } else {
               while (state.current_indention < state.indention_stack.back()) {
                  tokens.push_back(Token<TokenSetT>(dedention_token, state.indention_start.value(),
                                                    state.current_indention.value()));
   ...
   std::expected<void, ErrorType> end_lexing(IndentionRuleState& state, Tokens<TokenSetT>& tokens,
                                             const TextPos& pos) const {
